...
RNGkind(sample.kind = "Rejection")
tic()
ests_ver3 <- foreach (i = 1:n.sims, .combine = rbind) %dopar% {
...
mean(ls.r1.no.nan[,2]) # 6.272354

# Locate the minimum one. 
# minimum : 9039th 1.099495
...
info.list.r1 <- calculate.bw(ss.mat = ls.r1, lb.bw = 2.5, ub.bw = 2.8, 
