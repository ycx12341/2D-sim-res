...
# of T98G glioma invasion patterns. (error-calibrated version)

# Read in the cell densities data at day 1 and day 3.
library(readr)
glioma.ref.den <- read_rds("Glioma simulation full reference densities.rds")
t1.ref.den <- glioma.ref.den$t1.ref.den
t3.ref.den <- glioma.ref.den$t3.ref.den

################################################################################

generate.pattern <- function(par, init.cells.rad, prob.death, prob.prof) {
  # Purpose: Generate a 2D glioma cells invasion pattern with the given
  # parameters.
  
  # Arguments:
  #    par: parameters of the PDE model
  #    init.cells.rad: initial radius of the glioma spheroid
  ...
  # Create a 40*40 domain.
  h <- 1/39
  space.length.x <- round((1/h) + 1)
  space.length.y <- round((1/h) + 1)
  
  x <- seq(0, 1, length.out = space.length.x)
  y <- seq(0, 1, length.out = space.length.y)
  
  # Time discretization
  T <- 103.7
  # dt chosen as 0.0075 so the stability condition (dn < ((h^2)/4dt)) 
  # can be maintained.
  dt <- 0.0075
  # Total dimensionless timesteps and the dimensionless timesteps for one 
  # single day.
  timesteps <- round(T/dt)
  day.timesteps <- round(34.56/dt)
  ...
  for (i in 1:space.length.x) {
    for (j in 1:space.length.y) {
      radius <- sqrt(((x[i]-0.5)^2) + ((y[j]-0.5)^2))
      if (radius < init.cells.rad) {
        n0[i, j] <- exp(-(radius^2)/eps)
      } else {
        n0[i, j] <- 0
      }
    }
  ...
  # If the init.cells.rad parameter is too small and no cells were generated. 
  if (sum(n0) == 0) {
    return(NaN)
    break
  }
  ...
  n.cells <- length(which(n0.sort != 0))
  x.coord <- vector()
  y.coord <- vector()
  
  # Initial glioma cells will be allocated to the locations with the highest 
  # densities in the domain (central part).
  ...
  for (i in 1:timesteps) {
    ...
    if (i %% (day.timesteps) == 0) {
      ...
      for (q in 1:length(prof.cells)) {
        ...
        if (cell.position[1] == 1) {
          ...
          # Other cells that are not at the boundary (same reasoning is 
          # followed...)
        } else {
          left <- ind.position[left.position[1], left.position[2]]
          right <- ind.position[right.position[1], right.position[2]]
    ...
    if (i > (day.timesteps * (80/96))) {
      # Diffusion starts having an impact after a certain amount of time in 
      # day 1.
      n[2:(length(y) - 1), 2:(length(x) - 1)] <- n[2:(length(y) - 1), 2:(length(x) - 1)] * (1 + (r * dt) - (4 * dn * dt /(h ^ 2))) + 
      ...
      n[2:(length(y) - 1), 2:(length(x) - 1)] <-  n[2:(length(y) - 1), 2:(length(x) - 1)] * (1 + (r * dt))
    }
    ...
    # Compute the density matrix at the end of day 1 and day 3. 
    if (i == (day.timesteps * 1)) {
      density.mat.d1 <- matrix(0, nrow = length(y.cut), ncol = length(x.cut))
      ind.position.d1 <- ind.position
      for (ii in 1:length(y.cut)) {
        for (jj in 1:length(x.cut)) {
          ...
          density.mat.d1[ii, jj] <- length(which(ind.position.sub == 
                                                   1)) / ((mat.size) ^ 2)
        }
      }
      n.day1 <- n
  ...
  # Output
  return(list(den.mat.d3 = density.mat.d3, ind.position.d3 = ind.position.d3,
              den.mat.d1 = density.mat.d1, ind.position.d1 = ind.position.d1,
              n.day3 = n.day3, n.day1 = n.day1))
}

calculate.sse <- function(pars, init.cells.rad, prob.death, prob.prof) {
  ...
  temp.den.table <- generate.pattern(par = pars, init.cells.rad = init.cells.rad,
  ...
  if (is.na(temp.den.table)) {
    ...
    sse.t1 <- sum((temp.den.table$den.mat.d1 - t1.ref.den)^2)
    sse.t3 <- sum((temp.den.table$den.mat.d3 - t3.ref.den)^2)
    
    # Aggregate sum of square differences.
    diff.tot <- sum(sse.t1, sse.t3)
    
    # Output. 
    return(list(diff = diff.tot, 
                sse.t1 = sse.t1,
                sse.t3 = sse.t3,
                n.day1 = temp.den.table$n.day1, 
                n.day3 = temp.den.table$n.day3,
                den.mat.d1 = temp.den.table$den.mat.d1, 
                den.mat.d3 = temp.den.table$den.mat.d3,
                ind.position.d1 = temp.den.table$ind.position.d1,
                ind.position.d3 = temp.den.table$ind.position.d3
                
    ))
  }
}

# Calculate the correct bw that would yield the desirable ESS. (error
# -calibrated version)
calculate.bw <- function(ss.mat, min.sd.d1, min.sd.d3, ess.target, step.size) {
  ...
  # min.sd.d1: standard deviation of the discrepancy betwwen the best-fitted
  #            output and the observed data (day 1 data only).
  # min.sd.d3: standard deviation of the discrepancy betwwen the best-fitted
  #            output and the observed data (day 3 data only).
  ...
  # Rescale the least square differences using the SDs of the discrepancies
  # between the best-fitted output and the observed density. 
  sse.rescaled <- ss.mat.valid[,3]/(min.sd.d1 ^ 2) + 
    ss.mat.valid[,4]/(min.sd.d3 ^ 2)
  
  # Calculate and rescale the weights based on the rescaled discrepancies. (with
  # log transform)
  log.wt <- log(dchisq(sse.rescaled, df = 200)) - 99 * log(sse.rescaled)
  log.wt.rescaled <- log.wt - max(log.wt)
  wt.rescaled <- exp(log.wt.rescaled)

  # Calculate the bandwidth factor (between 0 and 1) for the rescaled weights 
  # which would give us the desirable ESS. 
  power <- seq(0, 1, by = step.size)
  
  # An empty vector to store the ESS for each bandwidth factor. 
  ess.vec <- rep(0, length = length(power))
  
  # For every bandwidth factor between 0 and 1:
  for (i in 1:length(power)) {
    # Rescale the weights to resampling probabilities. 
    resamp.prob.temp <- wt.rescaled ^ (power[i])
  ...
  # Find the tempering factor that can give an ESS which is the closest to 
  # the desirable one. 
  ...
  # Calculate the corresponding resampling probabilities using 
  # the bandwidth factor located previously. 
  resamp.prob.obj <- wt.rescaled ^ bw.obj
  
  # Output
  info.list <- list(info.mat = cbind(ss.mat.valid, sse.rescaled, log.wt, 
                                     log.wt.rescaled, wt.rescaled, 
                                     resamp.prob.obj),
                    ess.mat = ess.mat,
                    ess.obj = ess.obj,
                    bw.obj = bw.obj)
