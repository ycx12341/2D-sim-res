...
# obtained at the end of the simulation for post-day 9 SCC invasion pattern to
...
length(which(is.na(ls.r6[,2]))) # 11 singular values. 
...
for (i in 1:length(ls.r6.valid[,2])) {
  ...
  ls.temp <- list(den.mat.d9 = res.temp$den.mat.d9)
...
ls.r6.min <- read_rds("Round_6_paras3675_res.rds")
sd.r6.d9.min <- sd(ls.r6.min$den.mat.d9 - t9.ref.den)
...
for (i in 1:length(paras.r6.output)) {
  ...
  # original dataset. 
  for (j in 1:length(den.mat.d9.temp.unpert)) {
    #if(den.mat.d9.temp.unpert[j] != 0) {
     den.mat.d9.temp.pert[j] <- rnorm(1, den.mat.d9.temp.unpert[j], 
                                      sd = sd.r6.d9.min)
  ...
  paras.r6.output.pert[[i]] <- list(den.mat.d9.pert = den.mat.d9.temp.pert, 
                                    diff = sse.d9.temp)
...
for (i in 1:length(paras.r6.output.pert)) {
  ...
  for (j in 1:length(paras.r6.valid[,2])) {
    ...
    sse.d9.temp <- sum((res.temp$den.mat.d9 - 
                          paras.r6.output.pert.temp$den.mat.d9.pert)^2)
    # Append index, discrepancy into each row of the matrix
    diff.mat[j, 1:2] <- c(ls.r6.valid.pert[j, 1], sse.d9.temp)
...
est <- foreach (i = 1:length(ls.r6.diff.mat), .combine = rbind) %dopar% {
  ...
  lb.bw.temp <- 8.05 # 4.23
  ub.bw.temp <- 8.16 # 4.34
