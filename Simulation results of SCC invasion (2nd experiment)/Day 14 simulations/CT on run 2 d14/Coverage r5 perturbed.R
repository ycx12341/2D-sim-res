# Coverage r5 perturbed.R
# This .R file conducts the coverage test on the final parameter samples 
# obtained at the end of the simulation for post-day 14 SCC invasion pattern to
...
# save.sims.dir <- "Cov_results_r5"
save.sims.dir <- "Cov_results_r5_pert_all"
save.sims <- TRUE
if (save.sims) {
...
paras.r5 <- read.table("Round 5 parameters.txt", sep = "", header = TRUE)

# Directory which stores the simulation output. 
sims.dir <- "LS_results_r5"

# Check if there are still NaNs in the output from posterior sample
ls.r5 <- read.table("Round 5 Least Square.txt", sep = "", header = TRUE)
length(which(is.na(ls.r5[,2]))) # 52 singular values. 

# Remove the singular simulation output and their corresponding parameter 
# vectors. 
ls.r5.valid <- ls.r5[-which(is.na(ls.r5[,2])), ]
paras.r5.valid <- paras.r5[-which(is.na(ls.r5[,2])), ]
...
paras.r5.output <- vector(mode = "list")

for (i in 1:length(ls.r5.valid[,2])) {
  res.temp <- read_rds(paste0("./", sims.dir, 
                              "/Round_5_paras",ls.r5.valid[i,1],"_res.rds"))
  ls.temp <- list(den.mat.d14 = res.temp$den.mat.d14)
  paras.r5.output[[i]] <- ls.temp
  print(i)
  names(paras.r5.output)[i] <- paste0("ls_r5_",ls.r5.valid[i,1])
}

# Write the unperturbed model output into a .rds file. 
write_rds(paras.r5.output, "Full output paras r5 run 2 valid.rds")
...
ls.r5.min <- read_rds("Round_5_paras4566_res.rds")
sd.r5.d14.min <- sd(ls.r5.min$den.mat.d14 - t14.ref.den)

# Add perturbations (gaussian) to the final unperturbed output based on the
# SD calculated in the previous step. 
paras.r5.output.pert <- vector(mode = "list")

set.seed(874514)
RNGkind(sample.kind = "Rejection")
for (i in 1:length(paras.r5.output)) {
  ls.temp <- paras.r5.output[[i]]
  ...
  # original dataset. 
  for (j in 1:length(den.mat.d14.temp.unpert)) {
    #if(den.mat.d14.temp.unpert[j] != 0) {
     den.mat.d14.temp.pert[j] <- rnorm(1, den.mat.d14.temp.unpert[j], 
                                       sd = sd.r5.d14.min)
  ...
  paras.r5.output.pert[[i]] <- list(den.mat.d14.pert = den.mat.d14.temp.pert, 
                                    diff = sse.d14.temp)
  # Name the elements in the list with correct indices
  names(paras.r5.output.pert)[i] <- paste0("ls_r5_",ls.r5.valid[i,1],"_pert")
  print(i)
}

# Sort the sum of squared differences
ls.r5.valid.pert <- matrix(0, nrow = length(ls.r5.valid[,2])
                     ,ncol = 2)
for (i in 1:length(ls.r5.valid.pert[,2])) {
  ls.r5.valid.pert[i,] <- c(ls.r5.valid[i, 1], 
                            as.double(paras.r5.output.pert[[i]]$diff))
}
ls.r5.valid.pert.sort <- order(ls.r5.valid.pert[,2])
ls.r5.valid.pert.sort.min <- ls.r5.valid.pert.sort[1:200]

# Index in the full dataset (from 1 - 10000)
ls.r5.pert.sort.min.index <- ls.r5.valid.pert[ls.r5.valid.pert.sort[1:200], 1]

# Locate the corresponding 200 perturbed model outputs that have the 
# minimum discrepancy with the reference data.
paras.r5.output.pert.full <- paras.r5.output.pert
paras.r5.output.pert <- paras.r5.output.pert[ls.r5.valid.pert.sort.min]
...
ls.r5.diff.mat <- vector(mode = "list")
for (i in 1:length(paras.r5.output.pert)) {
  ...
  diff.mat <- matrix(0, nrow = length(paras.r5.valid[,2]), ncol = 2)
  paras.r5.output.pert.temp <- paras.r5.output.pert[[i]]
  for (j in 1:length(paras.r5.valid[,2])) {
    # Read in the unperturbed model output.
    res.temp <- paras.r5.output[[j]]
...
est <- foreach (i = 1:length(ls.r5.diff.mat), .combine = rbind) %dopar% {
  ...
  readr::write_rds(info.list.temp, path = paste0("./", save.sims.dir, 
                                                 "/info_list_r5_", 
                                                 ls.r5.pert.sort.min.index[i], ".rds"))
...
# single list. 
ls.r5.crage <- vector(mode = "list")
for (i in 1:length(ls.r5.diff.mat)) {
...
# write_rds(ls.r5.crage, "Coverage_test_disp_wt_prob_r5_pert_non_zero.rds")
write_rds(ls.r5.crage, "Coverage_test_disp_wt_prob_r5_pert.rds")
...
# write.table(cov.mat, "Coverage probabilities r5 non zero only.txt")
write.table(cov.mat, "Coverage probabilities r5.txt")

cov.mat <- read.table("Coverage probabilities r5 non zero only.txt", sep = "", header = TRUE)
...
# 1/8 parameters shown weak evidence against the coverage test!

# All data perturbed: 
# 7/8 parameters passed the coverage test!
# 1/8 parameters shown weak evidence against H0.
