...
# Read in the cell densities at day 3. 
library(readr)
...
t3.ref.den <- cbind(rev(t3.dat.p1[, 5]), rev(t3.dat.p2[, 5]), rev(t3.dat.p3[, 5]),
                    rev(t3.dat.p4[, 5]), rev(t3.dat.p5[, 5]), rev(t3.dat.p6[, 5]),
                    rev(t3.dat.p7[, 5]))/100

################################################################################

generate.pattern <- function(par, init.cells.cols, prob.death, prob.prof) {
  ...
  # Create a 48*28 domain (first 3 rounds only).
  # h <- 1/47
  ...
  # T <- 4.52
  # dt <- 0.004
  # dt chosen as 0.004 so the stability condition (dn < ((h^2)/4dt)) 
  # can be maintained.
  ...
  # densities in the domain (left boundary).
  ...
  ind.position.init <- ind.position
  ...
  return(list(den.mat.d3 = density.mat.d3, ind.position.d3 = ind.position.d3,
              ind.position.init = ind.position.init,
              n.den.d3 = n.day3,
              f.den.d3 = f.day3, m.den.d3 = m.day3
  ))
}

calculate.sse <- function(pars, init.cells.cols, prob.death, prob.prof) {
  ...
  temp.den.table <- generate.pattern(par = pars, init.cells.cols = init.cells.cols,
  ...
  if (is.na(temp.den.table)) {
    ...
    # Output. 
    return(list(diff = sse.t3,
                den.mat.d3 = temp.den.table$den.mat.d3, 
                ind.position.d3 = temp.den.table$ind.position.d3,
                ind.position.init = temp.den.table$ind.position.init,
                n.den.d3 = temp.den.table$n.den.d3,
                f.den.d3 = temp.den.table$f.den.d3,
                m.den.d3 = temp.den.table$m.den.d3
                ))
  }
}
