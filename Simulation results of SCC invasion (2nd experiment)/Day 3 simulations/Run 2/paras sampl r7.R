...
ests <- foreach (i = 1:n.sims, .combine = rbind) %dopar% {
  sse.res.temp <- calculate.sse(pars = paras.r7[i, 1:6], 
...
mean(ls.r7.no.nan[,2]) # 0.3233861
# minimum : 8920th 0.08690499 

# Write the least square differences into a .txt file.  
write.table(ls.r7, "Round 7 Least Square.txt")

# ls.r7 <- as.matrix(read.table("Round 7 Least Square.txt", sep = "",
#                               header = TRUE))

# Calculate the bandwidth factor which gives the desirable ESS.
info.list.r7 <- calculate.bw(ss.mat = ls.r7, lb.bw = 3.5, ub.bw = 4, 
