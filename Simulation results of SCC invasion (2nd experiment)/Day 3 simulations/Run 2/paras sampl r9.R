...
ests <- foreach (i = 1:n.sims, .combine = rbind) %dopar% {
  sse.res.temp <- calculate.sse(pars = paras.r9[i, 1:6], 
...
mean(ls.r9.no.nan[,2]) # 0.2188877
# minimum : 4346th 0.07045379

# Write the least square differences into a .txt file.  
write.table(ls.r9, "Round 9 Least Square.txt")

# ls.r9 <- as.matrix(read.table("Round 9 Least Square.txt", sep = "",
#                               header = TRUE))

# Calculate the bandwidth factor which gives the desirable ESS.
info.list.r9 <- calculate.bw(ss.mat = ls.r9, lb.bw = 4.3, ub.bw = 4.5, 
