...
# obtained at the end of the simulation for post-day 12 SCC invasion pattern to
...
length(which(is.na(ls.r6[,2]))) # 46 singular values.
...
for (i in 1:length(ls.r6.valid[,2])) {
  res.temp <- read_rds(paste0("./", sims.dir, 
                              "/Round_6_paras",ls.r6.valid[i,1],"_res.rds"))
  ls.temp <- list(den.mat.d12 = res.temp$den.mat.d12)
...
write_rds(paras.r6.output, "Full output paras r6 run 1 valid.rds")
...
ls.r6.min <- read_rds("Round_6_paras9282_res.rds")
sd.r6.d12.min <- sd(ls.r6.min$den.mat.d12 - t12.ref.den)
...
for (i in 1:length(paras.r6.output)) {
  ...
  # original dataset. 
  for (j in 1:length(den.mat.d12.temp.unpert)) {
    #if(den.mat.d12.temp.unpert[j] != 0) {
     den.mat.d12.temp.pert[j] <- rnorm(1, den.mat.d12.temp.unpert[j], sd = sd.r6.d12.min)
  ...
  paras.r6.output.pert[[i]] <- list(den.mat.d12.pert = den.mat.d12.temp.pert, 
                                    diff = sse.d12.temp)
...
for (i in 1:length(paras.r6.output.pert)) {
  ...
  for (j in 1:length(paras.r6.valid[,2])) {
    ...
    sse.d12.temp <- sum((res.temp$den.mat.d12 - 
                           paras.r6.output.pert.temp$den.mat.d12.pert)^2)
    # Append index, discrepancy into each row of the matrix
    diff.mat[j, 1:2] <- c(ls.r6.valid.pert[j, 1], sse.d12.temp)
...
est <- foreach (i = 1:length(ls.r6.diff.mat), .combine = rbind) %dopar% {
  ...
  lb.bw.temp <- 7.21 #5.35
  ub.bw.temp <- 7.32 #5.46 
...
# 6/8 parameters passed the coverage test! 
# 2/8 parameters shown weak evidence against the coverage test!

# All data perturbed: 
# 6/8 parameters passed the coverage test!
# 1/8 parameters shown weak evidence against H0.
# 1/8 parameters shown moderate evidence against H0. 