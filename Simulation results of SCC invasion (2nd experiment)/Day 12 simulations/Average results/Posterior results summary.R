...
# of the post-day 12 SCC pattern and calculate their Bhattacharyya distance with 
# the reference data. The final output which yields the minimum B-C distance was
# chosen as the final result of the post-day 12 SCC pattern's simulation, and 
# the initial condition for the post-day 14 SCC pattern's simulation. In 
...
paras.day12 <- rbind(paras.r6.run1.mean, paras.r6.run2.mean, 
                    paras.r6.run3.mean)

# B-C distance calculations. 
run1.den.mat <- paras.r6.run1.res$den.mat.d12
run2.den.mat <- paras.r6.run2.res$den.mat.d12
run3.den.mat <- paras.r6.run3.res$den.mat.d12

bcd.run1.ref <- 0.25 * (log(0.25 * (var(c(run1.den.mat))/var(c(t12.ref.den)) + 
                                      var(c(t12.ref.den))/var(c(run1.den.mat)) + 2))) + 
  0.25 * (((mean(run1.den.mat) - mean(t12.ref.den))^2)/(var(c(t12.ref.den)) + var(c(run1.den.mat))))

# 7.665821e-05

bcd.run2.ref <- 0.25 * (log(0.25 * (var(c(run2.den.mat))/var(c(t12.ref.den)) + 
                                      var(c(t12.ref.den))/var(c(run2.den.mat)) + 2))) + 
  0.25 * (((mean(run2.den.mat) - 
              mean(t12.ref.den))^2)/(var(c(t12.ref.den)) + var(c(run2.den.mat))))

# 0.0002925885

bcd.run3.ref <- 0.25 * (log(0.25 * (var(c(run3.den.mat))/var(c(t12.ref.den)) + 
                                      var(c(t12.ref.den))/var(c(run3.den.mat)) + 2))) + 
  0.25 * (((mean(run3.den.mat) - 
              mean(t12.ref.den))^2)/(var(c(t12.ref.den)) + var(c(run3.den.mat))))
...
write_rds(list(paras.day12.3runs = paras.day12, paras.day12.final.est = paras.r6.run1.mean),
          "Day 12 parameter estimates.rds")
...
points(x = seq(1,5,by = 1), y = ess.bw.run3[,2], pch = 10, lwd = 2, 
       col = "blue")
