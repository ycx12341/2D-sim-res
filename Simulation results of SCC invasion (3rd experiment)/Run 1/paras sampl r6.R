# paras sampl r6.R
# Author: Yunchen Xiao
# This .R file reads in the post-round 5 parameters to be evaluated for the 
...
paras.r6 <- as.matrix(read.table("Round 6 parameters.txt", 
                                 sep = "", header = TRUE))
...
mean(ls.r6.no.nan[,2]) # 5.194555
# minimum : 31458th 3.159069
...
info.list.r6 <- calculate.bw(ss.mat = ls.r6, lb.bw = 11.2, ub.bw = 11.3, 
                             ess.target = 8201.25, step.size = 0.01)

write_rds(info.list.r6, "Round 6 information list.rds")

# err.mes.mat <- matrix(0, nrow = length(ind.nan), ncol = 2)
# for (i in 1:length(ind.nan)) {
#   res.nan.temp <- read_rds(paste0("./", save.sims.dir, 
#                                   "/Round_6_paras",ind.nan[i],"_res.rds"))
#   err.mes.mat[i,] <- c(ind.nan[i], res.nan.temp$err.mess) 
# }
