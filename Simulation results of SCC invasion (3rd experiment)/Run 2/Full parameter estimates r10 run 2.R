# Full parameter estimates r10 run2.R
# Author: Yunchen Xiao
...
dn.vals.r10 <- paras.mean.mat.full[10, 1:7]
gamma.vals.r10 <- paras.mean.mat.full[10, 8:14]
rn.vals.r10 <- paras.mean.mat.full[10, 15:21]
eta.vals.r10 <- paras.mean.mat.full[10, 22:28]
dm.vals.r10 <- paras.mean.mat.full[10, 29]
alpha.vals.r10 <- paras.mean.mat.full[10, 30:36]
r.init.vals.r10 <- paras.mean.mat.full[10, 37]
p.ext.vals.r10 <- paras.mean.mat.full[10, 38]
p.mit.vals.r10 <- paras.mean.mat.full[10, 39:45]

paras.r10.mean.mat.full <- rbind(dn.vals.r10, gamma.vals.r10, rn.vals.r10,
                                eta.vals.r10, c(rep(dm.vals.r10, 5),rep(NA, 2)), 
                                alpha.vals.r10, c(rep(r.init.vals.r10, 5),rep(NA, 2)),
                                c(rep(p.ext.vals.r10, 5), rep(NA, 2)), p.mit.vals.r10)

rownames(paras.r10.mean.mat.full) <- c("dn.vals.r10", "gamma.vals.r10", "rn.vals.r10",
                                      "eta.vals.r10", "dm.vals.r10", "alpha.vals.r10",
                                      "r.init.vals.r10", "p.ext.vals.r10", "p.mit.vals.r10")

colnames(paras.r10.mean.mat.full) <- c("init.par", "p2", "p3", "p4", 
...
write.table(paras.mean.mat.full, "Full parameter estimates 10 rounds run 3.txt")
write.table(paras.r10.mean.mat.full, "Round 10 final full parameter estimates run 3.txt")
...
# dn
dn.quad.mod <- lm(dn.vals.r10[1:5] ~ x + x2)
...
# gamma 
gamma.quad.mod <- lm(gamma.vals.r10[1:5] ~ x + x2)
...
# rn 
rn.quad.mod <- lm(rn.vals.r10[1:5] ~ x + x2)
...
# eta 
eta.quad.mod <- lm(eta.vals.r10[1:5] ~ x + x2)
...
# alpha
alpha.quad.mod <- lm(alpha.vals.r10[1:5] ~ x + x2)
...
# p.mit 
p.mit.quad.mod <- lm(p.mit.vals.r10[1:5] ~ x + x2)
...
reg.est.vals <- cbind(fitted.vals.predict.dn.quad, 
                      ...
                      rep(dm.vals.r10, length(periods.cont)),
                      fitted.vals.predict.alpha.quad,
                      rep(r.init.vals.r10, length(periods.cont)),
                      rep(p.ext.vals.r10, length(periods.cont)),
