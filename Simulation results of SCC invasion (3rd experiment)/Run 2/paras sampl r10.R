# paras sampl r10.R
# Author: Yunchen Xiao
# This .R file reads in the post-round 9 parameters to be evaluated for the 
...
paras.r10 <- as.matrix(read.table("Round 10 parameters.txt", 
                                 sep = "", header = TRUE))
...
mean(ls.r10.no.nan[,2]) # 3.888797
# minimum : 19368th 2.595311
...
paras.r10.mean <- apply(paras.r10, 2, mean)

# Generate the final simulation output. 
set.seed(874512) 
RNGkind(sample.kind = "Rejection")
paras.r10.mean.res <- generate.pattern(par = paras.r10.mean[1:6], 
                                       init.cells.cols = paras.r10.mean[7],
                                       prob.death = paras.r10.mean[8], 
                                       prob.prof = paras.r10.mean[9],
                                       slopes = paras.r10.mean[10:21])

# Write the final output into an .rds file. 
write_rds(paras.r10.mean.res, "paras r10 average result.rds")
# paras.r10.mean.res <- read_rds("paras r10 average result.rds")

# Plot of individual cell positions. 
sse.t3.r10.mean.grid <- paras.r10.mean.res$ind.position.d3
cell.coord <- which(sse.t3.r10.mean.grid == 1, arr.ind = TRUE)
...
den.mat.d3 <- paras.r10.mean.res$den.mat.d3
den.mat.d6 <- paras.r10.mean.res$den.mat.d6
den.mat.d9 <- paras.r10.mean.res$den.mat.d9
den.mat.d12 <- paras.r10.mean.res$den.mat.d12
den.mat.d14 <- paras.r10.mean.res$den.mat.d14
